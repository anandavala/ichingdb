...
#    seed(datetime.now())
    simple_query = "SELECT DISTINCT consultation_id, query, lc.hexagram AS initial, CASE WHEN MOD(lc.hexagram, 2) = 1 THEN 'inspiration' ELSE 'manifestation' END AS initial_type, h.name AS initial_name, h.description AS initial_desc, h.gnostic AS initial_gnostic, pair.hexagram2 AS mirror, CASE WHEN MOD(pair.hexagram2, 2) = 1 THEN 'inspiration' ELSE 'manifestation' END AS mirror_type, h2.name AS mirror_name, h2.description AS mirror_desc, h2.gnostic AS mirror_gnostic, hidden.hexagram_id AS hidden, hidden.name AS h_name, hidden.description AS h_desc, hidden.gnostic AS h_gnostic, notes, conclusion FROM consultation AS C JOIN line_changes AS lc USING (line_1 , line_2 , line_3 , line_4 , line_5 , line_6) JOIN hexagram AS h ON h.hexagram_id = lc.hexagram, hexagram AS h2, pair, line_changes AS lch, hexagram AS hidden WHERE lc.chng_1 = 0 AND lc.chng_2 = 0 AND lc.chng_3 = 0 AND lc.chng_4 = 0 AND lc.chng_5 = 0 AND lc.chng_6 = 0 AND pair.hexagram1_id = lc.hexagram AND h2.hexagram_id = pair.hexagram2 AND lch.line_1 = C.line_2 AND lch.line_2 = C.line_3 AND lch.line_3 = C.line_4 AND lch.line_4 = C.line_3 AND lch.line_5 = C.line_4 AND lch.line_6 = C.line_5 AND lch.chng_1 = 0 AND lch.chng_2 = 0 AND lch.chng_3 = 0 AND lch.chng_4 = 0 AND lch.chng_5 = 0 AND lch.chng_6 = 0 AND hidden.hexagram_id = lch.hexagram AND consultation_id = %d;" % cid
    line_query = "SELECT C.consultation_id, lc2.hexagram AS result, h1.name AS result_name, h1.description AS result_desc, h1.gnostic AS result_gnostic, hl.h_line_position AS line, hlp.name AS line_name, hlp.meaning AS line_desc, hl.related_hexagram AS context, h3.name AS context_name, h3.description AS context_desc, h3.gnostic AS context_gnostic, hl.translation AS line_trans, hl.commentary AS line_comm, hl2.h_line_position AS m_line, hlp2.name AS m_line_name, hlp2.meaning AS m_line_desc, hl2.related_hexagram AS m_context, h4.name AS m_context_name, h4.description AS m_context_desc, h4.gnostic AS m_context_gnostic, hl2.translation AS m_trans, hl2.commentary AS m_comm FROM consultation AS C JOIN line_changes AS lc USING (line_1 , line_2 , line_3 , line_4 , line_5 , line_6) JOIN line_changes AS lc2 USING (line_1 , line_2 , line_3 , line_4 , line_5 , line_6) JOIN hexagram AS h ON h.hexagram_id = lc.hexagram JOIN hexagram AS h1 ON h1.hexagram_id = lc2.hexagram, hexagram AS h2, pair, hexagram_line AS hl, hexagram AS h3, hexagram_line AS hl2, hexagram AS h4, h_line_position AS hlp, h_line_position AS hlp2 WHERE lc.chng_1 = 0 AND lc.chng_2 = 0 AND lc.chng_3 = 0 AND lc.chng_4 = 0 AND lc.chng_5 = 0 AND lc.chng_6 = 0 AND lc2.chng_1 = C.chng_1 AND lc2.chng_2 = C.chng_2 AND lc2.chng_3 = C.chng_3 AND lc2.chng_4 = C.chng_4 AND lc2.chng_5 = C.chng_5 AND lc2.chng_6 = C.chng_6 AND hl.hexagram = lc.hexagram AND hl.related_hexagram = h3.hexagram_id AND ((hl.h_line_position = 1 AND lc2.chng_1 = 1) OR (hl.h_line_position = 2 AND lc2.chng_2 = 1) OR (hl.h_line_position = 3 AND lc2.chng_3 = 1) OR (hl.h_line_position = 4 AND lc2.chng_4 = 1) OR (hl.h_line_position = 5 AND lc2.chng_5 = 1) OR (hl.h_line_position = 6 AND lc2.chng_6 = 1)) AND pair.hexagram1_id = lc.hexagram AND hl2.hexagram = pair.hexagram2 AND hl2.h_line_position = 7 - hl.h_line_position AND h4.hexagram_id = hl2.related_hexagram AND h2.hexagram_id = pair.hexagram2 AND hl.h_line_position = hlp.h_line_position_id AND hl2.h_line_position = hlp2.h_line_position_id AND consultation_id = %d;" % cid
